...
function setup_sprite(loopOpt, bbox_offsetOpt, bbox_sizeOpt, img_src, max_frames, max_ticks, frame_size, src_offset) {
  ...
  let bbox_size$1 = Caml_obj.equal(bbox_size, [
        0,
        0
...
function make_bgd(context) {
  let params = setup_sprite(undefined, undefined, undefined, "bgd-1.png", 1, 0, [
        512,
        256
      ], [
        0,
...
function make$1(velOpt, accOpt, part_type, pos, ctx) {
  let vel = velOpt !== undefined ? velOpt : [
      0,
      0
    ];
  ...
  let params = make_type$1(part_type, ctx);
  let pos$1 = pair_to_xy(pos);
  let vel$1 = pair_to_xy(vel);
  let acc$1 = pair_to_xy(acc);
  return {
          params: params,
          part_type: part_type,
          pos: pos$1,
...
function make$2($staropt$star, $staropt$star$1, spawnable, context, param) {
  ...
  let id$1 = id !== undefined ? id : new_id(undefined);
  let obj = {
    params: params,
    pos: {
      x: param[0],
...
function spawn(spawnable, context, param) {
  let match = make$2(undefined, undefined, spawnable, context, [
        param[0],
        param[1]
      ]);
  let obj = match[1];
...
function normalize_pos(pos, p1, p2) {
  let match = p1.bbox_offset;
  let match$1 = p2.bbox_offset;
  let match$2 = p1.bbox_size;
  let match$3 = p2.bbox_size;
...
function update_player(player, keys, context) {
  ...
  let pl_typ = player.health <= 1 ? "SmallM" : "BigM";
...
function normalize_origin(pos, spr) {
  let p = spr.params;
  let match = p.bbox_offset;
  let match$1 = p.bbox_size;
  pos.x = pos.x - match[0];
  pos.y = pos.y - (match[1] + match$1[1]);
}

function collide_block(check_xOpt, dir, obj) {
  let check_x = check_xOpt !== undefined ? check_xOpt : true;
  switch (dir) {
    case "North" :
        obj.vel.y = -0.001;
        return ;
...
function evolve_enemy(player_dir, typ, spr, obj, context) {
  switch (typ) {
    case "Goomba" :
        ...
        return ;
    case "GKoopa" :
        let match = make$2(undefined, obj.dir, {
              TAG: "SEnemy",
              _0: "GKoopaShell"
            }, context, [
              obj.pos.x,
    ...
    case "RKoopa" :
        let match$1 = make$2(undefined, obj.dir, {
              TAG: "SEnemy",
              _0: "RKoopaShell"
            }, context, [
              obj.pos.x,
        ...
        let new_obj$1 = match$1[1];
        let new_spr$1 = match$1[0];
...
function evolve_block(obj, context) {
  dec_health(obj);
  let match = make$2(undefined, undefined, {
        TAG: "SBlock",
        _0: "QBlockUsed"
      }, context, [
        obj.pos.x,
...
function get_aabb(obj) {
  let spr = obj._1.params;
  let obj$1 = obj._2;
  let match = spr.bbox_offset;
  let box = obj$1.pos.x + match[0];
  let boy = obj$1.pos.y + match[1];
  let match$1 = spr.bbox_size;
  let sy = match$1[1];
  let sx = match$1[0];
  return {
          center: {
...
function kill(collid, ctx) {
  switch (collid.TAG) {
    ...
    case "Block" :
        ...
        let pos_0$1 = o$2.pos.x;
        let pos_1$1 = o$2.pos.y;
        let pos$1 = [
          pos_0$1,
          pos_1$1
        ];
...
function render(sprite, param) {
  let context = sprite.context;
  let match = sprite.params.src_offset;
  let match$1 = sprite.params.frame_size;
  let sw = match$1[0];
...
function in_viewport(v, pos) {
  let v_min_x = v.pos.x - 32;
  let v_max_x = v.pos.x + v.v_dim.x;
  let v_min_y = v.pos.y - 32;
...
function process_collision(dir, c1, c2, state) {
          ...
          case "Block" :
              ...
              if (dir === "North") {
                if (typeof t !== "object") {
                  ...
                  let updated_block = evolve_block(o2$4, context);
                  let spawned_item = spawn_above(o1$3.dir, o2$4, t._0, context);
                  collide_block(undefined, dir, o1$3);
                  return [
                          spawned_item,
                          updated_block
  ...
  switch (exit) {
    ...
    case 2 :
        ...
        let r2$1 = o2$1.vel.x === 0 ? evolve_enemy(o1$1.dir, t2, s2$1, o2$1, context) : (dec_health(o1$1), o1$1.invuln = 60, undefined);
        return [
                undefined,
                r2$1
              ];
...
function check_collisions(collid, all_collids, state) {
  ...
  while(true) {
    ...
    if (equals(collid, h)) {
      ...
      let dir = check_collision(collid, h);
...
function update_collidable(state, collid, all_collids) {
  ...
  let viewport_filter = in_viewport(state.vpt, obj.pos) || is_player(collid) || out_of_viewport_below(state.vpt, obj.pos.y);
...
function run_update_collid(state, collid, all_collids) {
  if (collid.TAG === "Player") {
    let o = collid._2;
    let keys = translate_keys(undefined);
    o.crouch = false;
    let match = update_player(o, keys, state.ctx);
    let player;
    if (match !== undefined) {
      let new_spr = match[1];
...
function generate_helper(blockw, blockh, cx, cy, context) {
  let block_locs = generate_block_locs(blockw, blockh, 0, 0, /* [] */0);
