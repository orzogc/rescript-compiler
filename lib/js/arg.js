  ...
  while(current.contents < argv.contents.length) {
    try {
      ...
      if (s.length >= 1 && Caml_string.get(s, 0) === /* '-' */45) {
        ...
        let consume_arg = (function(follow){
        ...
        return function treat_action(f) {
          switch (f.TAG) {
            ...
            case "Expand" :
                ...
                let arg$7 = get_arg(undefined);
                let newarg = Curry._1(f._0, arg$7);
                consume_arg(undefined);
      ...
      let m = Caml_js_exceptions.internalToOCamlException(raw_m);
      if (m.RE_EXN_ID === Bad) {
        throw convert_error({
                  TAG: "Message",
                  _0: m._1
...
function parse(l, f, msg) {
  try {
    ...
    let msg$1 = Caml_js_exceptions.internalToOCamlException(raw_msg);
