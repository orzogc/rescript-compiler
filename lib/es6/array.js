...
function init(l, f) {
  ...
  let res = Caml_array.make(l, Curry._1(f, 0));
  for(let i = 1; i < l; ++i){
    res[i] = Curry._1(f, i);
  }
  return res;
...
function map(f, a) {
  ...
  let r = Caml_array.make(l, Curry._1(f, a[0]));
  for(let i = 1; i < l; ++i){
    r[i] = Curry._1(f, a[i]);
  }
  return r;
...
function map2(f, a, b) {
  ...
  let r = Caml_array.make(la, Curry._2(f, a[0], b[0]));
  for(let i = 1; i < la; ++i){
...
function mapi(f, a) {
  ...
  let r = Caml_array.make(l, Curry._2(f, 0, a[0]));
  for(let i = 1; i < l; ++i){
...
function of_list(l) {
  ...
  let a = Caml_array.make(list_length(0, l), l.hd);
  let _i = 1;
  let _param = l.tl;
  while(true) {
    let param = _param;
...
let Bottom = /* @__PURE__ */Caml_exceptions.create("Array.Bottom");

function sort(cmp, a) {
  let maxson = function (l, i) {
  ...
  let trickle = function (l, i, e) {
    try {
      ...
      let i$2 = Caml_js_exceptions.internalToOCamlException(raw_i);
...
function stable_sort(cmp, a) {
  let merge = function (src1ofs, src1len, src2, src2ofs, src2len, dst, dstofs) {
  ...
  let isortto = function (srcofs, dst, dstofs, len) {
    for(let i = 0; i < len; ++i){
  ...
  let sortto = function (srcofs, dst, dstofs, len) {
    if (len <= 5) {
