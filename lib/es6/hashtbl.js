...
function resize(indexfun, h) {
  ...
  h.data = ndata;
  let insert_bucket = function (_cell) {
    while(true) {
      let cell = _cell;
      ...
      let data = cell.data;
      let next = cell.next;
      let cell$1 = inplace ? cell : ({
            TAG: "Cons",
            key: key,
            data: data,
            next: "Empty"
          });
      let nidx = Curry._2(indexfun, h, key);
...
function filter_map_inplace_bucket(f, h, i, _prec, _slot) {
  while(true) {
    ...
    let data = slot.data;
    let next = slot.next;
    let data$1 = Curry._2(f, key, data);
    if (data$1 !== undefined) {
      if (typeof prec !== "object") {
...
function MakeSeeded(H) {
  ...
  let replace_bucket = function (key, data, _slot) {
    while(true) {
      let slot = _slot;
      if (typeof slot !== "object") {
        return true;
